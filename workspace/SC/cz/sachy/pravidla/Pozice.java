...
public class Pozice {
  ...
  public void nalezPseudolegalniTahy() {
    ...
    if (bily) {
     ...
     case 2: /* kun*/
      for (j = 8; j <= 15; j++)
        if ((sch[i + ofsety[j]]) <=0 ) zaradTah(i,i + ofsety[j]);
      break;
     case 3: /* strelec*/
  ...
  private boolean jednoZnacny(int tah, int urcen)
   {int odkud, kam;
  ...
  public String tah2Str(int tah) {
   ...
   if (abs(sch[odkud]) != 1) {
     if (!jednoZnacny(tah, J_Nic)) /* Zkusim Da1 */
    {    /* Tak Dha1 */
      if (jednoZnacny(tah, J_Sloupec)) s.append((char)((odkud-a1)%10 + 'a'));
       else /* Tak D1a1 */
        if (jednoZnacny(tah, J_Radka)) s.append((char)((odkud-a1)/10 + '1'));
...
/* s[i++]=(odkud-a1)/10 + '1';*/
 ...
 return new String(s);
   ...
   * @return
   */
  public boolean JeTam1(int odkud) {
    //int p, k;
    
    Iterator ti = this.tahy.iterator(); 
    while (ti.hasNext()) {
  ...
  public boolean JeTam2(int odkud, int kam) {
    Iterator ti = this.tahy.iterator(); 
    while (ti.hasNext()) {
      int t = ((Integer)ti.next()).intValue();
      // Normalni tah
  ...
  public int DoplnTah(int odkud, int kam) {
    Iterator ti = this.tahy.iterator(); 
    while (ti.hasNext()) {
      int t = ((Integer)ti.next()).intValue();
       /* Normalni tah*/
    ...
    /*Promena bileho pesce*/
     if ((t>>12)==12)
       if(odkud==a7+((t>>7)&7) && kam==a8+((t>>4)&7))
         return(t&(0xFFFF^(3<<10)))|(MessageBox.messageBox("Promìna pìšce", "Vyberte figuru", MessageBox.MB_PROMENA)<<10);
       else continue;
    /*Promena cerneho pesce*/
  if ((t>>12)==13)
