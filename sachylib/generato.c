...
#define ZaradBilouPromenu(pzas,p1, p2, fig, k)\
 {\
  *(pzas)=(u16)((3<<14)|(fig<<10)|((p1-a7)<<7)|((p2-a8)<<4));\
  (pzas)++;\
 }
...
#define ZaradCernouPromenu(pzas,p1, p2, fig, k)\
 {\
  *(pzas)=(u16)((13<<12)|(fig<<10)|((p1-a2)<<7)|((p2-a1)<<4));\
  (pzas)++;\
 }
...
/**********************************************************/
  ...
  *a=(s8)0;
  for(i=0;i<8;i++)
   if ((*(a+ofsety[i]))<=0 && !Ohrozeno((a+ofsety[i]),0)){
  ...
  *a=(s8)0;
  for(i=0;i<8;i++)
   if ((*(a+ofsety[i]))>=0 && (*(a+ofsety[i]))<=6 && !Ohrozeno((a+ofsety[i]),1)){
   odkud=a-u->pozice.sch;
   kam=odkud+ofsety[i];
