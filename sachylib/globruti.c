...
{s16 i;
 ...
 for(i=uloha->zasobnik.hranice[uloha->zasobnik.pos];
  ...
  if(tah==uloha->zasobnik.tahy[i]) {uloha->zasobnik.pos--;return 1;}
...
{s16 i;
 ...
 for(i=uloha->zasobnik.hranice[uloha->zasobnik.pos];
     i<uloha->zasobnik.hranice[uloha->zasobnik.pos+1];i++)
  if(tah==uloha->zasobnik.tahy[i]) return 1;
 return 0;
}
 ...
 *   je-li useknout 1, je dalsim parametrem unsigned - tah, ktery ze se to *
 *    ma zahrat tah - a tim se taky preplacne pripadny ocas partie       *
 *   je-li useknout 0, nejsou zadne dalsi parametry, jde o           *
...
{TTah1 *p,*k;
 ...
 while(p<k)
  {p++;
  t=*p;
  if(!(t>>15)) /* Normalni tah*/  if  (odkud==t>>7)  return 1; else continue;
  if ((t==MBRoch || t==VBRoch)) if (odkud==e1) return 1; else continue;
  if ((t==MCRoch || t==VCRoch)) if (odkud==e8) return 1; else continue;
  /*Promena bileho pesce*/
  if ((t>>12)==12) if (odkud == a7+((t>>7)&7)) return 1; else continue;
  /*Promena cerneho pesce*/
  if ((t>>12)==13) if (odkud == a2+((t>>7)&7)) return 1; else continue;
  /* Brani mimochodem (nic jineho to uz byt nemuze)*/
  if ((t&0x3fff)>>7 == odkud) return 1; else continue;
  }
 return 0;
}
...
if (t==VCRoch) if(odkud==e8 && kam==c8) return 1; else continue;
  /*Promena bileho pesce*/
if ((t>>12)==12)
 if(odkud==a7+((t>>7)&7) && kam==a8+((t>>4)&7)) return 1; else continue;
  /*Promena cerneho pesce*/
if ((t>>12)==13)
 if(odkud==a2+((t>>7)&7) && kam==a1+((t>>4)&7)) return 1; else continue;
/* Brani mimochodem (nic jineho to uz byt nemuze)*/
t&=0x3fff; /* odstraneni prvnich dvou bitu, aby se lepe siftovalo*/
if(kam==(t&127) && odkud==(t>>7)) return 1; else continue;
} /* konec while cyklu*/
 return 0;
}
  ...
  if (t&(1<<15))
   {
    /* nenormalni tah */
     if (t&(1<<14))
      ...
      {
       if (t&(1<<13)) return 0; else return 1;
    ...
    /* nenormalni tah */
   {
    if (pos->sch[t&127] || pos->sch[t>>7]==1 || pos->sch[t>>7]==-1) return 1;
     else return 0;
   }
 }
